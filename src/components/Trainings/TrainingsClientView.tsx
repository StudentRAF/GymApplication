...
const TrainingsClientView = () => {
  ...
  const [orderBy,   setOrderBy]   = useState<TrainingClientHeadType>();

  const handleSortEvent = (newOrderBy: TrainingClientHeadType) => () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const data = await fetch(`http://localhost:8000/api/schedule/client-training-appointment?page=${currentPage - 1}&size=${pageSize}${(orderType !== 'none') ? orderBy ? `&sort=${trainingClientParamName[orderBy]},${orderType}`: '': ''}`, {
        method: 'GET',
        headers: {
  ...
  return (
    <Box sx={formStyles}>
      ...
      <Table size='medium'>
        <TableHead>
          <TableRow>
            {trainingClientHeadType.map((item, index) => (
              <TableCell key={item} width={trainingClientHeadWidth[index]} sx={{borderTop:'1px solid #515151'}}>
                  <TableSortLabel
...
export default TrainingsClientView;
